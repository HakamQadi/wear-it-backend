...
function build(rows: ItemRow[], composeResult: Promise<{ imageUrl: string }> = Promise.resolve({ imageUrl: '/uploads/look-1.png' })) {
  ...
  const billing = {
    reserveGeneration: jest.fn(async () => ({ usageId: new Types.ObjectId().toString() })),
    refundGeneration: jest.fn(async () => undefined),
...
describe('LooksService.generate', () => {
  ...
  it('rejects two items of the same clothing type before reserving quota', async () => {
    const first = item('white-tee', TEE_TYPE), second = item('black-tee', TEE_TYPE);
    const { service, tryOn, billing } = build([first, second]);
    await expect(service.generate(new Types.ObjectId().toString(), { itemIds: [first._id.toString(), second._id.toString()], photoId })).rejects.toThrow(/only include one T-shirt/);
    expect(tryOn.composeLook).not.toHaveBeenCalled();
    expect(billing.reserveGeneration).not.toHaveBeenCalled();
  });

  it('rejects the same item selected twice', async () => {
    const tee = item('tee', TEE_TYPE), { service } = build([tee]), id = tee._id.toString();
    await expectAppError(service.generate(new Types.ObjectId().toString(), { itemIds: [id, id], photoId }), HttpStatus.BAD_REQUEST, 'ITEM_DUPLICATE');
  });

  it('rejects a look referencing an item the member does not own', async () => {
    const mine = item('tee', TEE_TYPE), { service } = build([mine]);
    await expectAppError(service.generate(new Types.ObjectId().toString(), { itemIds: [mine._id.toString(), new Types.ObjectId().toString()], photoId }), HttpStatus.NOT_FOUND, 'ITEM_NOT_YOURS');
  });

  it('accepts one item per type and layers them in clothing-type order', async () => {
    const jacket = item('jacket', JACKET_TYPE), tee = item('tee', TEE_TYPE), pants = item('pants', PANTS_TYPE);
    const { service, tryOn, billing, created } = build([jacket, tee, pants]);
    const look = await service.generate(new Types.ObjectId().toString(), { itemIds: [jacket._id.toString(), tee._id.toString(), pants._id.toString()], photoId });
    expect((tryOn.composeLook as jest.Mock).mock.calls[0][0].garments.map((g: { label: string }) => g.label)).toEqual(['T-shirt', 'Pants', 'Jacket']);
    expect(billing.reserveGeneration).toHaveBeenCalledTimes(1);
    expect(created[0].status).toBe('ready');
    expect((look as unknown as Look).resultImageUrl).toBe('/uploads/look-1.png');
  });

  it('records a failed look, refunds quota and rethrows when generation fails', async () => {
    const tee = item('tee', TEE_TYPE);
    const { service, billing, created } = build([tee], Promise.reject(new BadRequestException('image service down')));
    await expect(service.generate(new Types.ObjectId().toString(), { itemIds: [tee._id.toString()], photoId })).rejects.toThrow('image service down');
    expect(billing.refundGeneration).toHaveBeenCalledTimes(1);
  ...
  it('refuses a second generation while one is still running', async () => {
    ...
    const pending = new Promise<{ imageUrl: string }>((resolve) => { release = resolve; });
    const { service } = build([tee], pending), userId = new Types.ObjectId().toString();
    const first = service.generate(userId, { itemIds: [tee._id.toString()], photoId });
    await Promise.resolve();
    await expect(service.generate(userId, { itemIds: [tee._id.toString()], photoId })).rejects.toThrow(/already being generated/);
  ...
  it('refuses an item whose clothing type was removed', async () => {
    const orphan: ItemRow = { ...item('tee', TEE_TYPE), typeId: null }, { service } = build([orphan]);
    await expect(service.generate(new Types.ObjectId().toString(), { itemIds: [orphan._id.toString()], photoId })).rejects.toThrow(/no clothing type/);
  });
});
